...
// que si copie usuario            
        ...
        public function usuario_update2($data)
        {
            ...
            try
            {
                ...
                if($data['contrasena'] == ''){
                    ...
                    $consulta->execute(['id_usuario' => $data['id_usuario'],'nombre' => $data['nombre'], 'apellido' => $data['apellido'], 'usuario' => $data['usuario']]);
                }
                else{
                    $contrasena = password_hash($data['contrasena'], PASSWORD_DEFAULT);
                    $consulta = $this->db->con->prepare("CALL actualizar_usuario (:id_usuario, :nombre, :apellido, :usuario, :contrasena)");
                    $consulta->execute(['id_usuario' => $data['id_usuario'], 'nombre' => $data['nombre'], 'apellido' => $data['apellido'], 'usuario' => $data['usuario'], 'contrasena' => $contrasena]);
                }
                $created = true;
            }
            catch(PDOException $e)
        ...
        public function venta()
        {
            ...
            try
            {
                ...
                $consulta->execute();
                while($fila = $consulta->fetch())
                    array_push($created, ['libro_nombre' => $fila['libro_nombre'], 'nombre_usuario' => $fila['nombre_usuario'], 'direccion' => $fila['direccion'], 'rfc' => $fila['rfc'], 'credito' => $fila['credito'], 'precio' => $fila['precio'], 'cantidad' => $fila['cantidad'], 'total' => $fila['total'], 'imagen' => $fila['imagen']]);
            }
            catch(PDOException $e)
            {
            }
       ...
       public function perfil($data)
       {
            ...
            try
            {
                ...
                $consulta->execute(['id_usuario' => $data['id_usuario']]);
                while($fila = $consulta->fetch())
                    $created = array(['id_usuario' => $fila['id_usuario'], 'nombre' => $fila['nombre'], 'apellido' => $fila['apellido'], 'usuario' => $fila['usuario'], 'fotografia' => $fila['fotografia']]);
            }
            catch(PDOException $e)
            {
            }
       ...
       public function  autor_buscar($data)
       {
        $created = array();
        try
        {
            $this->db->open_connection();
            if($data['palabra'] =='')
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_autor WHERE nombre");
            else
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_autor WHERE nombre like CONCAT('%', :palabra,'%')");
            $PALABRA=$data['palabra'];
            $resultado->bindParam(":palabra",$PALABRA, PDO::PARAM_STR);
            $resultado->execute();
            while($fila = $resultado->fetch())
                array_push($created, ['id_autor' => $fila['id_autor'], 'nombre' => $fila['nombre'], 'edad' => $fila['edad'], 'genero' => $fila['genero']]);
    //        array_push($created, ['nombre'=>$fila['nombre']]);
        }
        catch(PDOException $e)
        {
        }
       ...
       public function  libro_buscar($data)
       {
        $created = array();
        try
        {
            $this->db->open_connection();
            if($data['palabra'] =='')
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_libro WHERE nombre");
            else
    ...
    //        array_push($created, ['nombre'=>$fila['nombre']]);
       ...
       public function  cliente_buscar($data)
       {
        $created = array();
        try
        {
            $this->db->open_connection();
            if($data['palabra'] =='')
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_cliente");
            else{
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_cliente WHERE nombre like CONCAT('%', :palabra,'%') ");
            $PALABRA=$data['palabra'];
            $resultado->bindParam(":palabra",$PALABRA, PDO::PARAM_STR);
            }
            $resultado->execute();
    ...
    //        array_push($created, ['nombre'=>$fila['nombre']]);
       ...
       public function  usuario_buscar($data)
       {
        $created = array();
        try
        {
            $this->db->open_connection();
            if($data['palabra'] =='')
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_usuario");
            else
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_usuario WHERE nombre like CONCAT('%', :palabra,'%')");
            $PALABRA=$data['palabra'];
            $resultado->bindParam(":palabra",$PALABRA, PDO::PARAM_STR);
            $resultado->execute();
            while($fila = $resultado->fetch())
    ...
    //        array_push($created, ['nombre'=>$fila['nombre']]);
       ...
       public function  editorial_buscar($data)
       {
        $created = array();
        try
        {
            $this->db->open_connection();
            if($data['palabra'] =='')
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_editorial");
            else
                $resultado = $this->db->con->prepare("SELECT * FROM mostrar_editorial WHERE nombre like CONCAT('%', :palabra,'%')");
            $PALABRA=$data['palabra'];
            $resultado->bindParam(":palabra",$PALABRA, PDO::PARAM_STR);
            $resultado->execute();
            while($fila = $resultado->fetch())
